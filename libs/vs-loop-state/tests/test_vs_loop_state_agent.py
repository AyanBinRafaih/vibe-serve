...
def test_hypothesis_plan_text_round_trips(tmp_path: Path) -> None:
    ...
    assert reloaded.records[0].hypothesis_claim == (
        "batching prefill removes per-request launch overhead"
    )
    assert reloaded.records[0].hypothesis_task == "batch the prefill step"


def test_record_written_before_hypothesis_plan_text_loads(tmp_path: Path) -> None:
    path = tmp_path / "rounds.json"
    path.write_text(
        json.dumps(
            [
    ...
    assert reloaded.records[0].hypothesis_claim is None
    assert reloaded.records[0].hypothesis_task is None
