...
trap cleanup EXIT
"$tt" --session "$session" run --restart --backend alacritty --cols 150 --rows 40 --cwd "$worktree" --env TERM=xterm-256color bash /tmp/pr656-run.sh "$worktree" "$output_dir/$label-protocol.json"
"$tt" --session "$session" expect text "Rejected" --timeout 15000
