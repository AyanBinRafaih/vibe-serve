...
@pytest.fixture
def sink_events() -> Iterator[list[RunEvent]]:
...
def test_scripted_mode_publishes_the_whole_agent_event_vocabulary(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    assistant = [
        event
        for event in _of_type(sink_events, EventType.AGENT_OUTPUT_CHUNK)
    ...
    tool_calls = [
        event
        for event in _of_type(sink_events, EventType.TOOL_CALL)
        if isinstance(event.data, ToolCallData) and event.data.tool == "Bash"
    ]
    tool_results = _of_type(sink_events, EventType.TOOL_RESULT)
    todos = _of_type(sink_events, EventType.TODO_UPDATE)
    usage = _of_type(sink_events, EventType.USAGE_UPDATE)
...
def test_scripted_tool_results_carry_the_configured_payload_size(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    results = _of_type(sink_events, EventType.TOOL_RESULT)
...
def test_scripted_tool_calls_and_results_are_correlated(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    result_ids = [
        event.data.call_id
        for event in _of_type(sink_events, EventType.TOOL_RESULT)
...
def test_events_are_scoped_to_the_invoking_role_and_round(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    scoped = [event for event in sink_events if event.type is EventType.TOOL_CALL]
    assert scoped
...
def test_replay_mode_round_trips_a_recorded_event_log(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    def shape(events: list[RunEvent]) -> list[tuple[str, str]]:
        return [
...
def test_replay_mode_reproduces_assistant_and_analysis_text(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    def channels(events: list[RunEvent]) -> list[str]:
        return [
            event.data.channel
            for event in events
...
def test_replay_mode_skips_a_truncated_trailing_record(tmp_path, sink_events):  # noqa: ANN001, ANN201
    ...
    assert _of_type(sink_events, EventType.TOOL_CALL)


def _label(event: RunEvent) -> str:
    data = event.data
...
def _record(driver: MockDriver, workspace: Path) -> list[RunEvent]:
