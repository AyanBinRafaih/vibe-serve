...
def test_modal_environment_prompt_notes_cover_seeded_checkouts(tmp_path):
    ...
    notes = seeded.view.prompt_notes
    ...
    unseeded_dir.mkdir()
    unseeded = env.open(_request(unseeded_dir, backend, agent_backend="cli", cli_provider="codex"))
    assert "add_local_dir('vllm'" not in unseeded.view.prompt_notes
    assert "seeded starting-point" not in unseeded.view.prompt_notes.lower()
