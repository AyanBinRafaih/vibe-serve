...
The workspace was seeded with pinned source checkout(s) that the objective names as the starting point: `vllm/` (vllm, https://github.com/vllm-project/vllm @ d7de043d55d1). Treat seeded checkouts as implementer-owned code — ordinary mutable workspace code the implementer builds on — not as framework directories or read-only vendored dependencies.
