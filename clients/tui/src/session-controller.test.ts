...
describe('session controller', () => {
  ...
  it('runs a slash command typed in the chat through the main input path', async () => {
    ...
    // The performance plot, which is the command that answers with an overlay
    // now that the experiment log is reached without a command.
    ...
    expect(controller.state.overlay?.content).toContain('No performance data yet.');
